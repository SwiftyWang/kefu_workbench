...
class ChatReCordProvide with ChangeNotifier {
  ...
  Future<void> getAdmins() async{
    ...
    if (response.data["code"] == 200) {
      admins= (response.data["data"]['list'] as List).map((i){
        AdminModel  _admin = AdminModel.fromJson(i);
        if(selectedAdmin == null && _admin.id == GlobalProvide.getInstance().serviceUser.id){
          selectedAdmin = _admin;
        }
        return _admin;
      }).toList();
  ...
  Future<void> getServicesStatistical() async{
    ...
    isLoading = true;
    notifyListeners();
    Response response = await publicService.getServicesStatistical(pageOn: pageOn, pageSize: pageSize, cid: selectedAdmin?.id, date: date, isDeWeighting: isDeWeighting);
    isLoading = false;
    notifyListeners();
